...
def randomNumber(seed, minVal, maxVal):
    seed = (seed * 9301 + 49297) % 233280 # here we just wrote rnadom numbers like we can also can amke seed(42)
...
def generateRandomVector(size, sparsity, seed):
    vector = {}
    for i in range(size):
        rand, seed = randomNumber(seed, 0, 1) #here is like from 0 10  for example rundom(40, 0, 10)
        if rand > sparsity: # if 0.9>0.8
            value, seed = randomNumber(seed, -100, 100) # we make vector rnaod from -100 to 100 
            vector[i] = value
    return vector, seed
